...
> ⚠️ В репозиторий намеренно не включены реальные бинарники RustDesk и реальный публичный ключ сервера.  
> Положи их вручную по путям, указанным ниже.
...
Никакие реальные домены, IP, имена ПК, ключи и пароли в шаблон не включены.
...
## 9. Rollout-рекомендация

Не запускать сразу на все 120+ ПК.
...
Контроль вести по reports, а не по факту `gpupdate`.

---

## 10. Важное по gpupdate

`gpupdate /force` только подтягивает политику.  
Startup Script выполняется при старте ПК.
...
Если проверка утром, `gpupdate /force` необязателен: фоновые политики обычно обновятся за ночь, но нужен reboot/startup.

---

## 11. Что не хранить в Git

Не коммитить:

```text
- реальные RustDesk EXE/MSI бинарники, если не нужно
- реальный public key, если считаешь его чувствительным
- reports с паролями
- реальные deploy.log
- реальные password.txt со старых тестов
```
